...
const GL={window:globalThis,self:globalThis,document:doc,localStorage:lsm,innerWidth:1280,innerHeight:800,devicePixelRatio:1,performance:{now:()=>(pt+=16.7)},requestAnimationFrame:()=>1,cancelAnimationFrame(){},addEventListener(){},removeEventListener(){},matchMedia:()=>({matches:false,addEventListener(){},removeEventListener(){},addListener(){},removeListener(){}}),navigator:{userAgent:'node',hardwareConcurrency:4,language:'en-US',languages:['en-US']},screen:{width:1280,height:800},alert(){},confirm:()=>true,prompt:()=>'',Image:function(){return el('img')},OffscreenCanvas:function(w,h){const c=el('canvas');c.width=w;c.height=h;return c},setTimeout:()=>0,clearTimeout(){},setInterval:()=>0,clearInterval(){}};
...
body+=`\n;globalThis.__A={setLang,getLang:()=>LANG,fmtN,fmtFull,applyLang};`;
new Function(body)();
const A=globalThis.__A;
const mega=()=>cache['#btnMega'].textContent, lang=()=>cache['#btnLang'].textContent;
console.log('Start LANG:', A.getLang(), '| btnMega:', JSON.stringify(mega()), '| fmtN(2.5e9):', A.fmtN(2.5e9), '| fmtFull(12345):', A.fmtFull(12345));
A.setLang('de');
console.log('-> DE     LANG:', A.getLang(), '| btnMega:', JSON.stringify(mega()), '| btnLang:', lang(), '| fmtN(2.5e9):', A.fmtN(2.5e9), '| fmtFull(12345):', A.fmtFull(12345));
A.setLang('en');
console.log('-> EN     LANG:', A.getLang(), '| btnMega:', JSON.stringify(mega()), '| btnLang:', lang(), '| fmtN(2.5e9):', A.fmtN(2.5e9), '| fmtFull(12345):', A.fmtFull(12345));
const ok = A.getLang()==='en' && mega()==='Megastructures' && lang()==='EN' && A.fmtN(2.5e9)==='2.5B' && A.fmtFull(12345)==='12,345';
console.log(ok?'\n✓ i18n-Framework OK':'\n✗ FEHLER');
process.exit(ok?0:1);
